...
export default function App() {
  ...
  const page = {
    ...
    privacy: <Privacy settings={snapshot.settings} authenticated={snapshot.authenticated} used={snapshot.usage?.used ?? 0} limit={snapshot.usage?.limit ?? 100} onProvider={(provider) => run(() => bridge.setSetting("provider", provider))} onSaveKey={(key) => run(() => bridge.setByok(key), "API key guardada")} onDeleteKey={() => run(bridge.deleteByok, "API key eliminada")} onRequestCode={async (email) => { await bridge.requestHostedOtp(email); notify("Código enviado"); }} onVerify={async (email, code) => { await bridge.verifyHostedOtp(email, code); await refresh(); notify("Sesión iniciada"); }} onRetryStore={async () => { await bridge.retryStoreHostedSession(); await refresh(); notify("Sesión guardada"); }} onSignOut={() => run(bridge.signOut, "Sesión cerrada")} />,
